...
async def start(message: types.Message, user_service: IUserService, state: FSMContext):
    ...
    await message.reply(
        ...
        " Чтобы мы могли передать  подарок, пожалуйста, "
        "укажите ваши имя и номер телефона для связи, что бы мы могли делиться с"
        " вами  полезными и теплыми материалами для всей семьи: открытки к "
        "праздникам, раскраски с веселыми персонажами, "
        "журналы  полезных привычек и многое другое. "
        "Это и не только будет приятным дополнением к нашему общению!"
    )
